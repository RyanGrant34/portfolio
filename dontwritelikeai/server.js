...
// Rate limiter: FREE_DAILY_LIMIT requests per IP per day
...
// API endpoint
app.post("/api/rewrite", rewriteLimiter, async (req, res) => {
  const { text } = req.body;
  ...
  if (text.length > 5000) {
    return res
      .status(400)
      .json({ error: "Text must be under 5,000 characters." });
  }

  if (text.trim().length < 20) {
    return res
      .status(400)
      .json({ error: "Please provide at least 20 characters." });
  }
  ...
  try {
    ...
    if (err.status === 401) {
      return res
        .status(500)
        .json({ error: "API key not configured. Set ANTHROPIC_API_KEY." });
    }
    res.status(500).json({ error: "Something went wrong. Try again." });
  }
});

// Remaining uses endpoint
app.get("/api/remaining", (req, res) => {
  // express-rate-limit sets these headers
  const limit = res.getHeader("RateLimit-Limit") || FREE_DAILY_LIMIT;
  const remaining =
    res.getHeader("RateLimit-Remaining") || FREE_DAILY_LIMIT;
  res.json({ limit, remaining });
});

app.listen(PORT, () => {
  console.log(`dontwritelikeai running at http://localhost:${PORT}`);
});
